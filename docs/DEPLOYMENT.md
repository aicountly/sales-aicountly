...
| --- | --- | --- | --- |
| Deploy to cPanel Production | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://sales.aicountly.com (+ `/api`) |
| Deploy to cPanel Sandbox | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://sales.gh.aicountly.com (+ `/api`) |
...
`https://sales.aicountly.com/api/.env` unless Apache is told otherwise.
