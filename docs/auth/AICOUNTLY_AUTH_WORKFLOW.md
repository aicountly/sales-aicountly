# AICOUNTLY auth workflow (Sales)

How Sales signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `sales.aicountly.com` (or `sales.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/sales?returnUrl={origin}/auth/callback`.
...
| Sales host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `sales.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `sales.aicountly.com/api` |
| `sales.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `sales.gh.aicountly.com/api` |
...
`sales` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://sales.gh.aicountly.com/api/health
...
curl -i -X POST https://sales.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://sales.gh.aicountly.com/api/global/login   # expect 404
```
